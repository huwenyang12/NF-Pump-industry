...
def drop_unnamed_cols(df: pd.DataFrame) -> pd.DataFrame:
    ...
    bad_cols = [c for c in df.columns if c == "" or c.startswith("Unnamed")]
    if bad_cols:
        log.logger.warning(f"[台账清理] 检测到无效列：{bad_cols}，将删除后再回写")
...
def short_msg(msg: str, n: int = 50) -> str:
    s = (msg or "").strip()
    # 压缩空白，避免 Excel 里显示难看
    s = re.sub(r"\s+", " ", s)
    return s[:n]
...
def is_migo_done_row(row: dict) -> bool:
    """只要过账状态有值，都视为处理过"""
    status = str(row.get("过账状态", "") or "").strip()
    return status != ""
...
def handle_migo(site_config):
    ...
    for r in rows:
        ...
        if not so or so == "跳过":
            continue
        if not so.startswith("67"):
            log.logger.info(f"[MIGO] 销售凭证非67开头，跳过：SO={so}, PO={po}")
            continue
        if not po:
            continue
        ...
        if material_doc:
            ...
            reason = short_msg(migo_msg, 50) or "失败"
...
def is_vl02n_done_row(row: dict) -> bool:
    status = str(row.get("VL02N过账", "") or "").strip()
    return status != ""


def update_vl02n_into_file(订单记录_path: str, 销售凭证: str, 采购订单号: str, VL02N过账: str):
    """按 (SO+PO) 回写：VL02N过账"""
    date_str = dtParser.get_network_time().split()[0].replace('.', '-')
    mypath = os.path.join(订单记录_path, f"{date_str}.xlsx")
    if not os.path.exists(mypath):
    ...
    # 自动补列
    if "VL02N过账" not in df.columns:
        df["VL02N过账"] = ""

    for c in ["销售凭证", "采购订单号"]:
        if c not in df.columns:
            log.logger.error(f"[VL02N] 台账缺少列：{c}，无法回写。path={mypath}")
            return
    ...
    df.loc[mask, "VL02N过账"] = (VL02N过账 or "").strip()
    df.to_excel(mypath, index=False)


def handle_vl02n(site_config):
    """
    VL02N 退货库存转移过账：
    """
    record_dir = dtParser._normalize_record_folder(site_config["录单"]["订单记录路径"])
    ...
    rows = dtParser.load_订单记录文件(record_dir)
    if not rows:
        # log.logger.info("[VL02N] 台账为空，无需处理")
        return True

    for r in rows:
        so = (r.get("销售凭证", "") or "").strip()
        po = (r.get("采购订单号", "") or "").strip()
        no = (r.get("物料凭证", "") or "").strip()

        # 基本过滤
        if not so or so == "跳过":
            continue
        if not so.startswith("67"):
            log.logger.info(f"[VL02N] 销售凭证非67开头，跳过：SO={so}, PO={po}")
            continue
        if not po:
            continue

        # 只有 MIGO 成功(有物料凭证) 才能做 VL02N
        if not no or str(r.get("过账状态", "")).strip() != "成功":
            continue
        # 去重：VL02N 已处理则跳过
        if is_vl02n_done_row(r):
            continue
        ...
        items = snap.get("items", []) if snap else []
        if not items:
            log.logger.warning(f"[VL02N] 快照无 items：PO={po}, SO={so}")
            continue
        log.logger.info(f"[VL02N] 准备执行：SO={so}, PO={po}, 物料凭证={no}, item数={len(items)}")
        ...
        vl02n_msg = ""
        try:
            vl02n_msg = sap.vl02n_收货(so, no, items) or ""
        except Exception as e:
            vl02n_msg = f"VL02N异常：{e}"
            log.logger.error(f"[VL02N] 异常：SO={so}, PO={po}, err={e}")

        # 回写台账
        if "已保存" in str(vl02n_msg):
            update_vl02n_into_file(record_dir, so, po, "成功")
            log.logger.info(f"[VL02N] 成功：SO={so}, PO={po}")
        else:
            reason = short_msg(str(vl02n_msg), 50) or "失败"
            update_vl02n_into_file(record_dir, so, po, reason)
            log.logger.warning(f"[VL02N] 失败：SO={so}, PO={po}, 返回={vl02n_msg}")
