...
def jiaohuodan(site_config):
    if not site_config.__contains__("交货单"):
        return False
    video_folder = site_config["交货单"]["录屏保存路径"]

    try:
        # 企业微信通知
        #dingtalk_utils.send_message(f"{site_config['name']}: 开始处理交货单")
        #r = record.start_recorder(video_folder, "交货单.xlsx")
        shouhou_ludan.handle_jiaohuodan(site_config)
    except Exception as e:
        log.logger.error("异常 {}".format(traceback.format_exc()))
        raise e
    finally:
