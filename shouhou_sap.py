...
def check_11_hang(all_items):
    ...
    for row_idx, item in enumerate(all_items):
        try:
            ...
            if not (mat.startswith("11") or mat.startswith("100")):
                log.logger.info(f"第 {row_idx+1} 行物料: {mat} 非11或100开头, 跳过")
                continue

            # 分组选择正确的 locator
            if row_idx < 7:
                row_locator = locator.sap.MIGO.行选择区
                row_params = {"idx": row_idx}
            else:
            ...
            log.logger.info(f"第 {row_idx+1} 行物料: {mat} 点击成功)")
            time.sleep(1.2)
            ...
            result = cc.wait_appear(locator.sap.MIGO.查找序列号, wait_timeout=6)
            if result is None:
                log.logger.info(f"未发现'查找序列号'控件，跳过当前物料行：{mat}")
                continue

            # ======== 序列号填写 ========
            first_row = cc.wait_appear(locator.sap.MIGO.序列号填写行, {'idx': 0}, wait_timeout=6)
            if first_row is None:
                log.logger.warning("已点击'查找序列号'，但未出现序列号填写区域")
                continue
            ...
            for sn_idx, sn in enumerate(serial_list):
                input_text_simple(locator.sap.MIGO.序列号填写行, {'idx': sn_idx}, sn)
                time.sleep(0.3)
                log.logger.info(
                    f"第 {row_idx+1} 行物料 {mat}：填写序列号 {sn_idx+1}/{len(serial_list)} = {sn}"
                )

        except Exception as e:
            log.logger.error(f"第 {row_idx+1} 行物料11开头点击失败: {e}")
            raise
...
def migo_收货(so, all_items):
    ...
    while True:
        try:
            ...
            fail_text = ""
            # 失败/日志框
            fail_elem = cc.wait_appear(locator.sap.MIGO.显示过账日志, wait_timeout=10)
            if fail_elem:
                fail_text = (fail_elem.get_text() or "").strip()
                safe_click(locator.sap.MIGO.显示过账日志_继续)
                time.sleep(0.5)
            # 正常文本框：有控件且有内容 -> 返回正常内容；否则返回 fail_text
            ok_elem = cc.wait_appear(locator.sap.MIGO.过账文本框, wait_timeout=10)
            ok_text = (ok_elem.get_text() or "").strip() if ok_elem else ""

            if ok_text:
                log.logger.info("保存结果：" + ok_text)
                return ok_text

            log.logger.info("保存结果：" + fail_text)
            return fail_text
