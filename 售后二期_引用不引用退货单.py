...
def parse_order_excel(file_path):
    ...
    for idx, start_row in enumerate(block_starts):
        ...
        kehucankao = get_merged_value(ws, f"C{info_row}")  # 客户参考
        kehubianma = get_merged_value(ws, f"D{info_row}")  # 客户名称编码
        ...
        row = start_row + 1
        while row <= end_row:
            f_text = get_merged_value(ws, f"F{row}")  # 物料号列（核心列）

            # 结束标记：是否安装调试验收（模板里文字在F列区域）
            if "是否安装调试验收" in f_text:
                # 取下一行 F 列的值（你截图里就是 N）
                order_data["是否安装调试验收"] = get_merged_value(ws, f"F{row + 1}")

                # 备注1 / 备注2：G列为“备注1/备注2”，H列为值
                for r in range(row, min(row + 8, end_row + 1)):
                    g_val = get_merged_value(ws, f"G{r}")
                    if "备注1" in g_val:
                        order_data["备注1"] = get_merged_value(ws, f"H{r}")
                    if "备注2" in g_val:
                        order_data["备注2"] = get_merged_value(ws, f"H{r}")
                break
            ...
            item = {
                "项目行号": get_merged_value(ws, f"E{row}"),
                "物料号": get_merged_value(ws, f"F{row}"),
                "配件型号": get_merged_value(ws, f"G{row}"),
                "序列号": get_merged_value(ws, f"H{row}"),
                "订单数量": get_merged_value(ws, f"I{row}"),
                "金额": get_merged_value(ws, f"J{row}"),
                "工厂": get_merged_value(ws, f"K{row}"),
                "采购订单编号": get_merged_value(ws, f"L{row}"),
                "拒绝原因": get_merged_value(ws, f"M{row}"),
                "显示抬头详细信息": get_merged_value(ws, f"N{row}"),
                "物料文本": get_merged_value(ws, f"O{row}"),
            }
...
if __name__ == "__main__":
    file_path = r"D:\青臣云起\项目\南方流体模板解析\文件\一单多物料_引用不引用退货单模板.xlsx"
